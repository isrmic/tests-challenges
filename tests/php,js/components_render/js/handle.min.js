...
result!==null?result.innerHTML="Dados de usuário incorretos":null}).catch(function(error){console.log(error)})},logout(){localStorage.setItem('sessionLogin',!1)
localStorage.removeItem('username')},checkIsLoged(){var sessionLogin=localStorage.getItem('sessionLogin')
if(sessionLogin==="true")
return!0
else return!1},remove(elm){document.querySelector(elm).remove()},increment(){__GLOBAL_DATAsource.Count++},autoComplete(form){inputs=document.querySelector(form).querySelectorAll('input[type="text"]')
axios.get(`https://viacep.com.br/ws/${inputs[0].value}/json/`).then(function(response){if(!("error" in response.data)){inputs[1].value=response.data.logradouro
inputs[2].value=response.data.bairro
inputs[3].value=response.data.localidade
inputs[4].value=response.data.uf}
else console.log("Cep Inválido !")}).catch(function(error){console.log(error)})}}
